  ...
  return (
    ...
    >
      ...
      <div className="flex items-end gap-3 mt-4">
        {isNA ? (
          <span className={	ext-4xl font-bold  + (unavailable ? "text-textSecondary/40" : "text-textPrimary")}>
            N/A
          </span>
        ) : (
  ...
  const whyText = (() => {
    if (isHigh && gap != null && worstGroup && bestGroup) {
      ...
      return (
        ...
        " indicates your dataset treats these groups very differently."
      );
    }
    ...
    <div className="space-y-4">
      ...
      {metricsMode === "dataset_level" && (
        <div className="flex items-start gap-2 bg-blue-500/8 border border-blue-500/20 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-blue-300/80 leading-relaxed">
            <span className="font-semibold">Dataset-level analysis:</span> SPD and DI are measured from actual outcome distributions.
            EOD and AOD require model predictions and are not available for this analysis.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card
          ...
          term="EOD"
          value={eodUnavailable ? null : eod}
          delay={0.3}
          unavailable={eodUnavailable}
          unavailableReason="Model predictions required"
        />
        <Card
          title="Avg Odds Diff"
          term="AOD"
          value={aodUnavailable ? null : aod}
          delay={0.4}
          unavailable={aodUnavailable}
          unavailableReason="Model predictions required"
        />
      </div>
