"""
ByUs -- PDF Report Generator (v2 -- Professional Audit Edition)
...
def _cause_label(cause: str) -> str:
    return {"proxy": "PROXY DISCRIMINATION",
            "underrepresentation": "DATA UNDERREPRESENTATION",
            "historical_skew": "HISTORICAL BIAS",
            "none": "NO SIGNIFICANT BIAS"}.get(str(cause).lower(), str(cause).upper())
...
def _on_page(canvas, doc):
    ...
    canvas.drawString(MARGIN, fy - 5, "ByUs Bias Audit Report \u2014 Confidential")
...
class ReportGenerator:
    ...
    def generate(self, session_data: dict[str, Any]) -> bytes:
        ...
        mitigation       = session_data.get("mitigation_results", {})
        ...
        overall_sev      = bias_results.get("overall_severity", "unknown")
        metrics_per_attr = bias_results.get("metrics_per_attr", {})
        engine           = validation.get("engine", "ByUs Bias Engine")
        ...
        doc = BaseDocTemplate(
            ...
            title="ByUs Bias Audit Report", author="ByUs",
        )
    ...
    def _p1(self, filename, row_count, target_col, sensitive_attrs, scenario,
        ...
        story.append(Paragraph("ByUs", _S_LOGO))
        ...
        story.append(Paragraph(
            "This report was generated by ByUs automated bias analysis. "
    ...
    def _exec_summary(self, filename, row_count, scenario, sensitive_attrs,
        ...
        for attr, m in metrics_per_attr.items():
            ...
            gs = m.get("group_stats",{}); priv = m.get("privileged_group",""); unpriv = m.get("unprivileged_group","")
            if gs and priv and unpriv:
                pr = gs.get(str(priv),{}).get("positive_rate",0) or 0
                ur = gs.get(str(unpriv),{}).get("positive_rate",0) or 0
                gap = round(abs(pr-ur)*100,1)
                finding = (f"The most significant finding is that '{priv}' achieves a positive "
                           f"outcome rate of {pr*100:.1f}% compared to only {ur*100:.1f}% for "
                           f"'{unpriv}' \u2014 a {gap} percentage-point gap on the '{attr}' attribute.")
                break

        winner = (mitigation or {}).get("winner","")
        wr = {"reweigh":"Reweighing","threshold":"Threshold Adjustment"}.get(winner,"")
        w_eff = (mitigation or {}).get("reweigh" if winner=="reweigh" else "threshold",{}).get("effects",{})
        br = w_eff.get("bias_reduction_pct",0)
        ...
        parts = [
            f"This audit analyzed <b>{_comma(row_count)}</b> records from <b>{filename}</b> "
            f"in the context of <b>{str(scenario).lower()}</b>. The dataset was assessed across "
            f"<b>{n}</b> sensitive attribute(s): <b>{', '.join(sensitive_attrs)}</b>. "
    ...
    def _p2(self, scenario, metrics_per_attr, pattern_preds, row_count) -> list:
        ...
        for attr, m in metrics_per_attr.items():
            ...
            priv  = m.get("privileged_group",""); unpriv = m.get("unprivileged_group","")
            ...
            met_rows = [
                ...
                [Paragraph("Statistical Parity Difference (SPD)",_S_BODY),
                 ...
                 Paragraph(f"Outcome rate gap between groups<br/>{_spd_ctx(spd)}",_S_BODY)],
            ...
            story.append(Paragraph(
                f'<font color="{GRAY.hexval()}">{conf:.0f}% confident based on '
                f'{int(pred.get("training_examples",20))} analyzed cases</font>',
                _s("cct", fontName="Helvetica", fontSize=8, textColor=GRAY, leading=12, alignment=1)))
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph(self._cause_para(cause, attr, priv, unpriv, gs, prox), _S_BODY))
    ...
    def _plain_finding(self, attr, scenario, sev, gs, priv, unpriv, spd) -> str:
        ...
        return (
            ...
            f"The group '<b>{priv}</b>' achieves a positive outcome rate of <b>{pr*100:.1f}%</b>, "
            f"while '<b>{unpriv}</b>' achieves only <b>{ur*100:.1f}%</b> "
            f"\u2014 a <b>{gap} percentage-point</b> difference. "
    ...
    def _cause_para(self, cause, attr, priv, unpriv, gs, prox) -> str:
        pd2 = gs.get(str(priv),{}); ud2 = gs.get(str(unpriv),{})
        ...
        if cause == "proxy":
            return (
                ...
                f"This means the model can discriminate indirectly using this feature even if "
            ...
            return (
                ...
                f"\u2014 for example, hiring or lending decisions made under discriminatory norms. "
            ...
            return (
                f"The analysis did not detect a dominant bias pattern for '{attr}'. "
                f"Fairness metrics are within acceptable ranges. "
                f"Continue monitoring with quarterly audits as new data is collected."
            )
    ...
    def _p3(self, sensitive_attrs, mitigation) -> list:
        ...
        win_reason = mitigation.get("winner_reason","")
        rw = mitigation.get("reweigh",{}); th = mitigation.get("threshold",{})
    ...
    def _p4(self, sensitive_attrs, metrics_per_attr, mitigation, pattern_preds) -> list:
        ...
        steps.append((n2, f"Apply {wr} Before Model Retraining",
            f"Our analysis shows <b>{wr}</b> achieves <b>{br4:.0f}%</b> bias reduction for "
            f"'<b>{first_attr}</b>' while retaining <b>{ar4:.0f}%</b> of model accuracy. "
            f"Apply this technique to your training pipeline before the next model update.",
            "SHORT-TERM", AMBER)); n2+=1

        # Step 4: Monitoring
        steps.append((n2, "Establish Ongoing Fairness Monitoring",
            "Set up quarterly bias audits using ByUs to track fairness metrics over time. "
        ...
        for attr, m in metrics_per_attr.items():
            ...
            if pred.get("predicted_cause")=="underrepresentation":
                gs2b = m.get("group_stats",{}); unp2 = m.get("unprivileged_group","")
    ...
    def _p5(self, training_count) -> list:
        ...
        story.append(Paragraph("How ByUs Detects Bias", _S_H2))
        story.append(Paragraph(
            "ByUs uses four legally-recognized fairness metrics to evaluate bias across demographic "
        ...
        story.append(Paragraph(
            f"This report is based solely on the uploaded dataset. Findings reflect patterns in "
            f"historical data and may not capture all forms of bias. The auto-learning system draws "
            f"on <b>{training_count}</b> previously analyzed datasets to improve root cause predictions. "
            f"This report does not constitute legal advice. Consult qualified legal counsel before "
            f"making compliance decisions based on this report.", _S_BODY))
        story.append(Spacer(1, 3*mm))

        story.append(Paragraph("About ByUs", _S_H2))
        story.append(Paragraph(
            "ByUs is an AI-powered bias detection and mitigation platform built to make AI fairness "
            "accessible to all organizations. It uses Google Gemini 2.0 Flash AI, Microsoft Fairlearn, "
        ...
        story.append(Paragraph("Generated by ByUs  |  Team Radiant  |  NMIMS Indore",
            _s("ff1", fontName="Helvetica-Bold", fontSize=8, textColor=TEAL, leading=12, alignment=1)))
        story.append(Paragraph("This report is confidential and intended for internal use only.",
